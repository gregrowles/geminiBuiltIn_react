...
// ---------------------------------------------------------------------------
// Types for the Chrome built-in LanguageModel API (Gemini Nano)
// ---------------------------------------------------------------------------

export interface LanguageModelSession {
  prompt(input: string): Promise<{ summary?: string } | string>;
...
export interface GeminiDefaults {
  systemPrompt: string;
}

export type StatusCallback = (message: string) => void;
export type ChunkCallback = (accumulatedText: string) => void;
...
export class GeminiPrompt {
  ...
  promptLanguageModel: LanguageModelSession | null = null;
   ...
   *
   * @param opts     Options forwarded to LanguageModel.create()
   ...
   */
  async init(opts: LanguageModelCreateOptions = {}, onStatus?: StatusCallback): Promise<void> {
    ...
    try {
      ...
      const msg = `LanguageModel failed to initialise: ${(e as Error).message}`;
      if (onStatus) onStatus(msg);
      throw e;
    }
   ...
   *
   * @param promptInput
   * @returns The full model response string
   */
  async prompt(promptInput: string): Promise<string> {
    this._assertReady();
    try {
      this.running = true;
   ...
   * @param onComplete  Called once with the final full text when the stream ends
   */
  async promptStream(
    promptInput: string,
